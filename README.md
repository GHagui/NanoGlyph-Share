...
- maximum Brotli compression for every newly generated link;
...
6. Select the platform through which the link will be sent.
...
9. **Compression:** the binary payload always uses Brotli quality 11 and receives a byte identifying the codec.
10. **Text and URL:** the result is converted with the Base62 alphabet `0-9A-Za-z` and appended to the canonical URL fragment.

New links always use maximum Brotli compression at quality 11. The decoder continues to support adaptive Brotli and Zlib links generated by older NanoGlyph versions.
...
├── i18n.js / locales/        # Locale resolver and translation catalogs
...
4. generates the English and Brazilian Portuguese web manifests;
5. copies the HTML, CSS, Worker, icons, and HEIF dependency;
6. injects the `package.json` version into the bundle and Service Worker cache name.

`./build.sh` is a shortcut for `npm run build:web`.

After the first build, `npm run dev` starts the development server on port 8081. It watches JavaScript, locale catalogs, HTML, CSS, the Worker, and manifests, then reloads connected browsers automatically.
...
The Rust tests cover the header and flags, pixel packing, RLE, Base62, rotation, and session-cache invalidation. The JavaScript tests exercise payloads with hundreds of parts, the full 8 MiB budget, rejection of invalid metadata, locale resolution, catalog parity, placeholders, and HTML translation keys.

### Contributing a translation

English in [`locales/en.js`](./locales/en.js) is the reference catalog and the runtime fallback. To add a locale:

1. copy the English catalog, translate only message values, and keep its `code`, native `name`, and flat message map structure;
2. register the new catalog in [`locales/index.js`](./locales/index.js);
3. preserve every message key and every placeholder exactly, including names such as `{index}`, `{total}`, and `{filename}`;
4. add the localized manifest filename to the Service Worker cache when the build will generate a new manifest variant;
5. run `npm test` to validate key and placeholder parity, the web tests, Rust tests, localized manifests, and the complete build.

Catalog messages contain plain text only. Formatting belongs in HTML, where separate elements can receive their own `data-i18n` or translated attribute key.
...
| `npm run dev` | Serves `dist/` on port 8081 with automatic browser reload |
| `npm run test:rust` | Runs the crate tests |
| `npm run test:web` | Runs the Node tests |
| `npm run test:build` | Validates generated localized manifests and offline assets |
