...
pub fn encode_image(
    ...
    let header = NanoGlyphHeader::new(
        new_w as u16,
        new_h as u16,
        best_palette_id,
        is_animation,
...
fn compress_brotli_with_quality(data: &[u8], quality: u32) -> Result<Vec<u8>, String> {
    let mut compressed = Vec::new();
    {
        let mut enc = BrotliCompressor::new(&mut compressed, 4096, quality, 22);
...
pub(crate) fn compress(data: &[u8], mode: u8) -> Result<Vec<u8>, String> {
    match mode {
        0 => compress_zlib(data),
        2 => compress_brotli_with_quality(data, 11),
        _ => {
            let quality = if data.len() <= 32 * 1024 {
                11
...
pub(crate) fn base62_encode(input: &[u8]) -> String {
    ...
    while num > zero {
        ...
        chunks.push(remainder.to_u64().unwrap_or(0));
        num = quotient;
    }

    let mut result = String::with_capacity(leading_zeroes + chunks.len() * CHUNK_DIGITS);
    result.extend(std::iter::repeat_n('0', leading_zeroes));

    for (chunk_index, mut chunk) in chunks.into_iter().rev().enumerate() {
        let minimum_digits = if chunk_index == 0 { 1 } else { CHUNK_DIGITS };
        let mut digits = [b'0'; CHUNK_DIGITS];
        let mut cursor = CHUNK_DIGITS;
        while chunk > 0 {
            cursor -= 1;
            digits[cursor] = ALPHABET[(chunk % 62) as usize];
            chunk /= 62;
        }
        cursor = cursor.min(CHUNK_DIGITS - minimum_digits);
        result.push_str(std::str::from_utf8(&digits[cursor..]).expect("base62 is ASCII"));
    }
...
pub(crate) fn find_best_palette(img: &RgbaImage) -> u8 {
    ...
    let step = pixel_count.div_ceil(MAX_SAMPLES).max(1);

    for id in 0..99 {
        let pal = get_palette(id);
        let mut error = 0u64;

        for pixel in img.pixels().step_by(step) {
            let [r, g, b, _] = pixel.0;
            ...
            for c in &pal {
                ...
                if (dist as u32) < closest_dist {
                    closest_dist = dist as u32;
                }
            }
            error = error.saturating_add(closest_dist as u64);
        }
